...
import { saveCheckpoint, loadCheckpoint } from '../checkpoint/save.js';
...
import { defaultChannelTypeRegistry } from '../channels/registry.js';
...
import { PostureStateMachine, classifyToneOfMessage, rollupToneWindow } from '../posture/machine.js';
...
export class ThunderGateRuntime {
    ...
    wal;
    // Posture state machine + channel-type registry. Built in start()
    // once provenance exists so the machine can log decision changes.
    posture;
    channelTypes = defaultChannelTypeRegistry;
    // Rolling window of recent inbound-message tone classifications. Fed
    // by the inbound path on every turn; collapsed into a ToneTrend
    // before each posture compute. Bounded so a noisy day can't grow it.
    toneWindow = [];
    static TONE_WINDOW_LIMIT = 10;
    /**
    ...
    async start() {
        ...
        // PostureStateMachine reads WorldState before each turn, writes a
        // provenance row on every state change. Constructed here so it
        // exists before the first inbound (channels start below).
        this.posture = new PostureStateMachine(this.provenance);
        ...
        if (replay.recovered === 0 && replay.corrupted === 0) {
            ...
            const counts = Object.entries(replay.byType)
                ...
                .map(([t, n]) => `${t}=${n}`)
                .join(' ');
            console.log(`  ✓ WAL replay: ${replay.recovered} rows recovered ` +
                `(${counts || 'none'})` +
        ...
        if (this.config.ghost.enabled) {
            try {
                this.ghost = new GhostHarness(this.config, (input, history, opts) => this.shadowResponse(input, history, opts), this.buildGhostStateSnapshotSource());
     ...
     * same channel id.
     */
    async handleChannelInbound(entry) {
        const channelId = entry.channel.replace(/^thundercommo:/, '');
        // Classify the raw channel key into a typed ChannelType. Adapters
        // MAY pre-tag entry.channelType themselves; the registry fills it
        // in here when missing so older inbound paths keep working.
        const channelType = entry.channelType ?? this.channelTypes.classify(entry.channel);
        ...
        // ── Posture: update WorldState fields, detect overrides, recompute ──
        try {
            // 1. Manual override detection — runs before compute so the
            //    new override applies on the same turn Michael says it.
            const trig = this.posture.detectOverride(entry.text);
            if (trig) {
                const senderLower = (entry.sender || '').toLowerCase();
                this.posture.setOverride(trig, senderLower || 'michael');
            }
            // 2. Update WorldState posture inputs from this inbound.
            this.world.activeChannel = channelType;
            this.world.activeDevice = deviceForChannel(channelType);
            const prevInboundAt = this.world.lastInboundAt;
            this.world.interArrivalMs = prevInboundAt == null ? null : Math.max(0, entry.timestamp - prevInboundAt);
            this.world.lastInboundAt = entry.timestamp;
            // 3. Roll the tone window forward, collapse to a trend.
            this.toneWindow.push(classifyToneOfMessage(entry.text));
            if (this.toneWindow.length > ThunderGateRuntime.TONE_WINDOW_LIMIT) {
                this.toneWindow.shift();
            }
            this.world.toneTrend = rollupToneWindow(this.toneWindow);
            // 4. Compute posture. The machine stamps the decision onto
            //    world.posture and writes a provenance row if it changed.
            this.posture.computeFromWorld(this.world, { now: entry.timestamp });
        }
        catch (err) {
            console.warn('  ⚠ posture.compute skipped:', err.message);
        }
        // ── Frame lifecycle: advance/transition based on this inbound ──
        let frameDecision = null;
        try {
            ...
            console.warn('  ⚠ frame.onInbound skipped:', err.message);
        }
        ...
        try {
            ...
            console.warn('  ⚠ promises.onInbound skipped:', err.message);
        }
        // ── Untrain conversational trigger ──
        let untrainLine = '';
        try {
            if (detectUntrainTrigger(entry.text)) {
                const actor = (entry.sender || '').toLowerCase() === 'michael' ? 'michael' : 'jon';
            ...
            console.warn('  ⚠ untrain trigger skipped:', err.message);
        }
        let response;
        try {
        ...
        if (untrainLine)
            composedText = `${untrainLine}\n\n${composedText}`;
        if (surfaceLine)
            composedText = `${surfaceLine}\n\n${composedText}`;
        ...
        try {
            ...
            console.warn('  ⚠ promise extraction skipped:', err.message);
        }
        const delivery = {
            id: newMessageId(),
            agentId: 'jon',
     ...
     */
    async shadowResponse(input, history, opts) {
        ...
        return {
            ...
            lastInbound: () => {
                ...
                try {
                    if (!this.db)
                        return null;
                    const row = this.db.raw().prepare(`SELECT created_at, payload FROM memory_wal
     ...
     */
    async callGhostLLM(system, userInput, history = [], stateSnapshot) {
        ...
        try {
            ...
            console.warn('  ⚠ Ghost LLM transport error:', err.message);
            return '';
        }
    }
    /**
    ...
    async normalProcess(message) {
        ...
        if (!message.ghost) {
            try {
                ...
                console.warn('  ⚠ learning.onTurn skipped:', err.message);
            }
        }
        return response;
    }
     ...
     */
    async processLocalInference(message) {
        ...
        try {
            ...
            this.provenance.append({
                ...
                target: 'message',
                reason: `local inference threw: ${err.message}`
            });
    ...
    async localInferenceRagStub(_message) {
    ...
    backgroundPreprocessStub(_message) {
    ...
    async callLLM(messages) {
        ...
        try {
            ...
            console.warn('  ⚠ callLLM transport error:', err.message);
            return '';
        }
    }
    /**
 ...
 */
/**
 * Map a ChannelType to the device-class hint the posture machine reads.
 * Phone-class channels (ThunderCommo iOS, WhatsApp) → 'phone'; browser
 * extension and Slack → 'laptop' (no glance-shape pressure); CLI/TUI →
 * 'cli' so future rules can split that out from desktop typing.
 */
function deviceForChannel(t) {
    switch (t) {
        case ChannelType.THUNDERCOMMO:
        case ChannelType.WHATSAPP:
            return 'phone';
        case ChannelType.SLACK:
        case ChannelType.THUNDERBROWSER:
            return 'laptop';
...
function readLastGitCommit(repo) {
    try {
...
function readPrinciplesCount(repo) {
